...
pub fn get_extended_path() -> String {
    ...
    if platform::is_windows() {
        ...
        if let Some(home) = dirs::home_dir() {
            ...
            paths.push(format!("{}\\AppData\\Roaming\\nvm\\current", home_str));
            paths.push(format!("{}\\AppData\\Local\\Volta\\bin", home_str));
            paths.push(format!("{}\\AppData\\Roaming\\fnm", home_str));
            paths.push(format!("{}\\.fnm", home_str));
        }
    } else {
...
/// 异步执行命令并在超时后终止，供耗时 CLI（例如 npx skills）使用
pub async fn run_command_capture_with_timeout(
    ...
    let output = timeout(timeout_duration, command.output())
        .await
        .map_err(|_| format!("命令执行超时（>{} 秒）", timeout_duration.as_secs()))?
